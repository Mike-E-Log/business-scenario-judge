...
- **Decide what counts as success before you see the score.** I wrote the passing rule down before the judges ran, in a plan that cannot change afterward ([PREREGISTRATION.md](PREREGISTRATION.md), the pre-run plan). Like agreeing on the finish line before the race. The rule here: pass only if the calibrated judge matches my rulings on more of the 15 test chats than the uncalibrated judge does; matching on the same number counts as a fail. A rule fixed in advance cannot be bent to fit a weak score.
