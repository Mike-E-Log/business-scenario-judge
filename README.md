...
- **Fast labels are noisy labels.** Picking A better, B better, or tie is quick: all 60 rulings took one sitting. But re-grading blind days later, I matched my own rulings on only 12 of 15 chats. Every judge score inherits that noise, which is why the grader gets measured too ([Experiment 1](#experiment-1-the-self-check-grading-twice)).
- **Do not write the answer key first.** Each pair carried a planted flaw, so the "right" answer was decided before any grading. My blind rulings tracked something else: clarity, instruction-following, completeness ([the reason tags](metrics/results.md)), and the planted-flaw check in [Limitations](#limitations) found no measurable link. "Error analysis is the most important activity in evals" ([same FAQ](https://hamel.dev/blog/posts/evals-faq/)): read real outputs first, find the failures that actually happen, then build the labels from those.
- **Honesty needs machinery, not promises.** The plan and the pass bar were committed before the judges ran ([PREREGISTRATION.md](PREREGISTRATION.md)). When a review found a published number with no committed data behind it, the fix was committing the data, not softening the sentence. Automated tests now fail this page if its claims drift from the data files.

**The judge did not earn trust here. The way of finding that out did.**
