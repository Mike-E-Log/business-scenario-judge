...
- **Decide what counts as success before you see the score.** Before the judges ran, I wrote down the rule for passing in a plan that does not change afterward ([PREREGISTRATION.md](PREREGISTRATION.md), the pre-run plan; researchers call this preregistration). It works like agreeing where the finish line is before the race starts, not after seeing who leads. Here the rule was: the calibrated judge must beat the uncalibrated one outright, and a tie fails. With the rule fixed first, a weak score cannot be dressed up as a win. Tests guard this page the same way: if a claim drifts from the saved data, the build fails.
