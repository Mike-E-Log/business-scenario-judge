...
- **Define the success criteria before you see the score.** I wrote them down before the judges ran, in a plan that cannot change afterward ([PREREGISTRATION.md](PREREGISTRATION.md), the pre-run plan). The criteria here: the calibrated judge passes only by matching my rulings on more of the 15 test chats than the uncalibrated judge; the same number is a fail. By this rule the run passed, 8 chats to 6; why that is still not a real win is in [The honest result](#the-honest-result). Criteria fixed in advance cannot be bent to fit a weak score.
