...
- **Why that is not a win yet:** 15 chats is a small test, so luck alone can move these numbers a lot. The calibrated judge's true skill could sit anywhere from about 27% to 80%. The uncalibrated judge's could sit anywhere from about 13% to 67% (a bootstrap 95% range, `metrics/results.json`; [plain words below](#the-bootstrap-range-in-plain-words)). The two ranges overlap heavily. This one run cannot show the calibration truly helped.
- **The paired test agrees.** The plan committed to comparing the two judges chat by chat, not just side by side. That number is saved too. The gap's plausible range runs from 0 to 33 points, touching zero. An exact test on the 2 chats where only one judge was right gives p = 0.5 (`metrics/results.json`). Same verdict: not settled.
