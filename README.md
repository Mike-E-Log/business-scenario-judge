...
- **Decide what counts as success before you see the score.** I wrote the passing rule down before the judges ran, in a plan that cannot change afterward ([PREREGISTRATION.md](PREREGISTRATION.md), the pre-run plan). Like agreeing on the finish line before the race. The rule here: the calibrated judge must beat the uncalibrated one, and a tie fails. A rule fixed in advance cannot be bent to fit a weak score.
